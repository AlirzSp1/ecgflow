...
Modified for ECGFlow (https://github.com/4dm-labs/ecgflow)
"""
import argparse
import json
import logging
...
from timm.utils import ApexScaler, NativeScaler, freeze, unfreeze

from ecgflow.models import mvtst, mtsm  # custom models
from ecgflow import loss as ecgflow_loss
...
parser.add_argument('--ssl', default=False, action='store_true',
                    help='enable self-supervised learning mode (ignore targets)')
parser.add_argument('--ssl-mask-ratio', type=float, default=0.6, 
                    help='mask ratio for self-supervised learning')
...
group.add_argument('--notch-freq', default=60., type=float,metavar='N',
                   help='notch freq (Hz) (default: "60.0")')
group.add_argument('--bandpass-freq', default=(0.01, 150), nargs=2, type=float,
                   metavar='N N',
                   help='bandpass freqs (Hz) (default: "0.01 150")')
group.add_argument('--filter-wf', action='store_true', default=False, 
                   help='apply bandpass filter to waveforms (default: False)')
...
group.add_argument('--scale-y', default=False, action='store_true',
                   help=('scale target using training mean and SD '
                         'always True for MSE loss'))
...
group.add_argument('--ft-head', default=False, action='store_true',
                   help='freeze model except head to fine-tune (linear probe)')
group.add_argument('--ft-top', default=0, type=int, metavar='N',
                   help='freeze model up to top N layers to fine-tune')
group.add_argument('--keep-head', default=False, action='store_true',
                   help='keep pretrained head when fine-tuning')
...
group.add_argument('--mse-loss', action='store_true', default=False,
                   help='Enable MSE loss for continuous target')
group.add_argument('--mslq-loss', action='store_true', default=False,
                   help='Enable MSLQ loss for continuous target')
...
def main():
    ...
    if args.ssl:
        assert args.model.startswith('mtsm_'), 'SSL only supports "mtsm" architecture currently (see https://github.com/4dm-labs/ecgflow)'
        _logger.info('Self-supervised training mode enabled.')
    ...
    args.ecgflow_input = False
    dataset_parts = args.dataset.split('/')
    if dataset_parts[0] == 'ecgflow':
        args.ecgflow_input = True

    in_chans = 3
    if args.ecgflow_input:
        # 3 values required for `input-size`: ignore leading value
        in_chans = args.input_size[1]
    ...
    if args.pretrained_path:
        ...
        # head adaptation is forced when num_classes is -1.
        factory_kwargs['pretrained_cfg_overlay'] = dict(
            file=args.pretrained_path,
            num_classes=args.num_classes if args.keep_head else -1,
        )
    ...
    if args.ft_top > 0:
        _logger.info(f'Freezing all model layers except top {args.ft_top} '
                     'layer(s) and classifier head')        
        if model._get_name() in ('Eva', 'VisionTransformer'):
            utils.freeze(model)
            assert args.ft_top < len(model.blocks) - 1, f'invalid `ft_top` {args.ft_top}'
            utils.unfreeze(model, 'head')
            utils.unfreeze(model, 'fc_norm')
            for j in range(args.ft_top):
                utils.unfreeze(model.blocks[-(j+1)])
        elif model._get_name() == 'NormFreeNet':
            utils.freeze(model)
            utils.unfreeze(model, 'head')
            utils.unfreeze(model, 'final_conv')
            if args.ft_top > 1:
                _submods = [n for n,_ in model.named_modules()
                            if n.startswith('stages.3.8')]
                utils.unfreeze(model, _submods)
                _logger.info(f'\tUnfreezing top NF Block {_submods}')
        else:
            _logger.info(
                f'warning: freeze not supported for model {model._get_name()} ')
                    
    elif args.ft_head:
        _logger.info(f'Freezing all model layers except classifier head')
        if model._get_name() in ('Eva', 'VisionTransformer'):
            utils.freeze(model)
        elif model._get_name() == 'NormFreeNet':
            utils.freeze(model)
            ...
            _logger.info(
                f'warning: freeze not supported for model {mode._get_name()} ')
        utils.unfreeze(model, 'head')
    ...
    dataset_train = create_dataset(
        ...
        bandpass_freq=args.bandpass_freq,
        filter_wf=args.filter_wf,
    ...
    loader_train = create_loader(
        ...
        ecgflow_input=args.ecgflow_input,
    )
    ...
    if args.mse_loss:
        ...
        train_loss_fn = ecgflow_loss.MSLQLoss()
        validate_loss_fn = ecgflow_loss.MSLQLoss().to(device=device)
        args.eval_metric = 'loss'
        args.scale_y = False
    elif args.jsd_loss:
    ...
    if args.bce_loss:
        validate_loss_fn = BinaryCrossEntropy(smoothing=args.smoothing).to(device=device)
    elif not args.mse_loss and not args.mslq_loss:
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if eval_metrics is not None:
                ...
                if args.bce_loss:
                    latest_roc = eval_metrics.pop('ROC', None)
            else:
            ...
            if saver is not None:
                ...
                if args.bce_loss:
                    best_metric, best_epoch = saver.save_checkpoint(
                        epoch, metric=latest_metric, roc=latest_roc)
                else:
                    best_metric, best_epoch = saver.save_checkpoint(
...
def train_one_epoch(
    ...
    ssl = args.ssl  # self-supervised training
    ...
    for batch_idx, batch_d in enumerate(loader):
        ...
        def _forward():
            with amp_autocast():
                if ssl:
                    loss, _, _ = model(input, mask_ratio=args.ssl_mask_ratio)
                else:
                    output = model(input)
...
def validate(
    ...
    if not ssl and not args.mse_loss and not args.mslq_loss:        
        if ecgflow_input:
            pass # bin_m is an input argument
        else:
    ...
    with torch.no_grad():
        for batch_idx, batch_d in enumerate(loader):
            ...
            if args.distributed:
                ...
                if not ssl and not args.mse_loss and not args.mslq_loss:
                    if ecgflow_input:
                        pass  # nothing special to do (.compute method handles it)
                    else:
    ...
    if ssl or args.mse_loss or args.mslq_loss:
        metrics = OrderedDict([('loss', losses_m.avg)])
    elif ecgflow_input:
        metrics = OrderedDict([('loss', losses_m.avg),
                               ('ROC', roc)])
        metrics.update(bin_d)
    else:
        metrics = OrderedDict([('loss', losses_m.avg),
                               ('top1', top1_m.avg),
                               ('top5', top5_m.avg)])
